...
const validateArgs = (args: ChangeResponseStatusArgs) => {
  if (!range(100, 999).contains(args.from)) {
    throw new Error(
      `Invalid source HTTP response status (--from): ${args.from}. Must be in between 100 and 999.`,
    );
  }
  if (!range(100, 999).contains(args.to)) {
    throw new Error(
      `Invalid target HTTP response status (--to): ${args.to}. Must be in between 100 and 999.`,
    );
  }
};
...
export default async function changeResponseStatus(
  args: ChangeResponseStatusArgs,
) {
  validateArgs(args);
  ...
  for (const endpoint of endpoints) {
    ...
    for (const method of methods) {
      ...
      const { from, to } = args;
      const responsesData = ((paths[endpoint] ?? {})[method] ?? {})[
        "responses"
      ];
      ...
      if (!responsesData) {
        console.warn(
          `Skipping, missing 'responses' field. endpoint=${endpoint} method=${method}`,
        );
        continue;
      }
      ...
      if (!(toString(from) in responsesData)) {
        console.warn(
          `Skipping, missing source response status. endpoint=${endpoint} method=${method} from=${from}`,
        );
        continue;
      }
      ...
      if (toString(to) in responsesData) {
        throw new Error(
          `Refusing to overwrite existing response status. endpoint=${endpoint} method=${method}`,
        );
      }
